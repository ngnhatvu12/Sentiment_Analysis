    ...
    try:
        cur.execute("""
            SELECT EXISTS (
                ...
                WHERE table_name = 'api_sentiment_all_content'
            )
        """)
        ...
        if not table_exists:
            create_table_query = """
            CREATE TABLE api_sentiment_all_content (
            ...
            cur.execute("CREATE INDEX idx_api_sentiment_all_content_timestamp ON api_sentiment_all_content(timestamp)")
            cur.execute("CREATE INDEX idx_api_sentiment_all_content_source ON api_sentiment_all_content(source)")
            cur.execute("CREATE INDEX idx_api_sentiment_all_content_symbol ON api_sentiment_all_content(symbol)")
            cur.execute("CREATE INDEX idx_api_sentiment_all_content_sentiment ON api_sentiment_all_content(sentiment)")
            
            conn.commit()
            print("api_sentiment_all_content table created successfully!")
        else:
            print("api_sentiment_all_content table already exists")
...
def create_news_statistics_table():
    """Tạo bảng api_sentiment_news_statistics nếu chưa tồn tại"""
    conn = get_db_connection()
    cur = conn.cursor()
    ...
    try:
        cur.execute("""
            SELECT EXISTS (
                SELECT FROM information_schema.tables 
                WHERE table_name = 'api_sentiment_news_statistics'
            )
        """)
        ...
        if not table_exists:
            create_table_query = """
            CREATE TABLE api_sentiment_news_statistics (
                id SERIAL PRIMARY KEY,
                timestamp BIGINT NOT NULL,
            ...
            cur.execute("CREATE INDEX idx_api_sentiment_news_statistics_timestamp ON api_sentiment_news_statistics(timestamp)")
            
            conn.commit()
            print("api_sentiment_news_statistics table created successfully!")
        else:
            print("api_sentiment_news_statistics table already exists")
